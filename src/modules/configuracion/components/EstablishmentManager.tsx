...
import { BuildingOfficeIcon, PlusIcon, CheckCircleIcon, XMarkIcon, TagIcon, MapPinIcon } from '@heroicons/react/24/outline';
...
export default function EstablishmentManager({ businessInfo, onNotify }: EstablishmentManagerProps) {
  ...
  const handleCreateEstablishment = (e: React.FormEvent) => {
    ...
    const created: Establishment = {
      ...
      isMain: false,
      emissionPoints: [
        { id: `pts-${Date.now()}`, code: '001', description: 'Caja 1', isActive: true }
      ]
    };
          ...
          <div key={est.id} className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-6 border border-slate-100 dark:border-slate-700/50 shadow-sm space-y-4">
            ...
            <div className="border-t border-slate-100 dark:border-slate-700/60 pt-4 space-y-3">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1">
                <TagIcon className="w-3.5 h-3.5" /> Puntos de Emisión / Cajas Habilitadas
              </h4>
