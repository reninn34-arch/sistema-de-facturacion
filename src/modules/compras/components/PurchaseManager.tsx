            ...
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
              ...
              {manualSupplier ? (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label htmlFor={`${fieldId}-supplierName`} className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Nombre Proveedor</label>
                    <input id={`${fieldId}-supplierName`} value={supplierName} onChange={e => setSupplierName(e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none border-2 border-transparent focus:border-sky-500" placeholder="Nombre del proveedor" />
                  </div>
                  <div>
                    <label htmlFor={`${fieldId}-supplierRuc`} className="text-[10px] font-black text-slate-400 uppercase tracking-widest">RUC Proveedor</label>
                    <input id={`${fieldId}-supplierRuc`} value={supplierRuc} onChange={e => setSupplierRuc(e.target.value.replace(/\D/g, '').slice(0, 13))} className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none border-2 border-transparent focus:border-sky-500 font-mono" placeholder="1790012345001" maxLength={13} />
                  </div>
                </div>
              ) : (
                <div>
                  <label htmlFor={`${fieldId}-supplierSearch`} className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Buscar Proveedor</label>
                  <input
                    id={`${fieldId}-supplierSearch`}
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label htmlFor={`${fieldId}-docNumber`} className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Nro Comprobante</label>
                  <input id={`${fieldId}-docNumber`} value={docNumber} onChange={e => setDocNumber(e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none border-2 border-transparent focus:border-sky-500" placeholder="001-001-0000001" />
                </div>
                <div>
                  <label htmlFor={`${fieldId}-docDate`} className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fecha</label>
                  <input id={`${fieldId}-docDate`} type="date" value={docDate} onChange={e => setDocDate(e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none border-2 border-transparent focus:border-sky-500" />
                </div>
                <div>
                  <label htmlFor={`${fieldId}-accessKey`} className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Clave Acceso (opcional)</label>
                  <input id={`${fieldId}-accessKey`} value={accessKey} onChange={e => setAccessKey(e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl font-mono text-xs outline-none border-2 border-transparent focus:border-sky-500" placeholder="49 dígitos" />
                </div>
              </div>

              <div>
                <div className="flex justify-between items-center mb-3">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Items del Comprobante</span>
                ...
                {items.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {items.map((item, idx) => (
                      <div key={item.productId} className="flex gap-2 items-center bg-slate-50 p-3 rounded-xl">
                        <input
