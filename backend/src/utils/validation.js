/**
 * Utilidades de validación para pruebas
 * Versión JavaScript de utils/validation.ts
 */
...
const validateModulo11Special = (numero, coef, checkDigit) => {
  let suma = 0;
...
/**
 * Valida identificador ecuatoriano
 */
...
const validatePhone = (phone) => {
  if (!phone) return { valid: false, message: 'Teléfono es requerido' };
  
  // Eliminar espacios y guiones
  const cleanPhone = phone.replace(/[\s-]/g, '');
  
  // Verificar formato ecuatoriano
  // Ajustado para aceptar celulares (09xxxxxxxx, 10 dígitos) y fijos (0Yxxxxxxx, 9 dígitos)
  const ecuadorPhoneRegex = /^(09\d{8}|0[2-7]\d{7})$/;
  
  if (!ecuadorPhoneRegex.test(cleanPhone) && !/^\+5939\d{8}$/.test(cleanPhone)) {
...
const validateCI = (ci) => {
  ...
  return validateEcuadorianId(ci) 
    ? { valid: true } 
    : { valid: false, message: 'Cédula inválida' };
};
